...
export default function DayflowApp(){
  ...
  function getAllocations(empId){
    return LEAVE_TYPES.filter(lt=>lt.allocated!=null).map(lt=>{
      const a = leaveAllocations.find(x=>x.employeeId===empId && x.leaveTypeId===lt.id) || {allocated:lt.allocated, used:0};
  ...
  async function checkIn(empId){
    ...
    // Only attempt live Supabase backend call if empId is a real authenticated UUID
    if (isUuid(empId)) {
    ...
    // Default safe local state path for mock prototype sessions ("emp_1")
...
function PrivateInfoGrid({ emp }){
  ...
  const skillsText = Array.isArray(p.skills) ? p.skills.join(", ") : (p.skills || "—");
  const certsText = Array.isArray(p.certifications) ? p.certifications.join(", ") : (p.certifications || "—");
  return (
    <InfoGrid>
...
function LeaveHistoryTable({ rows, allocations }){
  return (
    <div>
      {allocations && (
      ...
      {rows.length===0 ? <EmptyState title="No time-off requests" subtitle="Requests will show up here."/> : (
        <div className="df-card" style={{padding:"6px 20px 14px"}}>
          <table className="df-table">
            <thead><tr><th>Type</th><th>Dates</th><th>Reason</th><th>Status</th><th>Comment</th></tr></thead>
            <tbody>
              {rows.map(r=>(
                <tr key={r.id}>
                  <td>{LEAVE_TYPES.find(t=>t.id===r.leaveTypeId)?.name}</td>
                  ...
                  <td style={{color:"var(--ink-faint)", fontSize:12.5}}>{r.comment||"—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function AdminTimeOffPage({ leaveRequests, employees, decideLeave }){
    ...
    <div>
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td>
                      ...
                      ) : <span style={{fontSize:12, color:"var(--ink-faint)"}}>{r.comment||"—"}</span>}
                    </td>
                  </tr>
                );
              })}
      ...
      {decisionModal && (
        <DecisionModal data={decisionModal} onClose={()=>setDecisionModal(null)}
          onConfirm={(comment)=>{ decideLeave(decisionModal.req.id, decisionModal.decision, comment); setDecisionModal(null); }}/>
      )}
    </div>
  );
}
  ...
  return (
    <Modal onClose={onClose} width={440}>
      ...
      <div style={{padding:"0 22px 22px", display:"flex", gap:10, justifyContent:"flex-end"}}>
        ...
        <button className={`df-btn ${approve?"df-btn-success":"df-btn-danger"}`} onClick={()=>onConfirm(comment)}>Confirm {data.decision.toLowerCase()}</button>
      </div>
    </Modal>
  );
}
  ...
  return (
    <div style={{display:"flex"}}>
      ...
      <div style={{flex:1, minWidth:0, padding:"28px 34px", maxHeight:"100vh", overflowY:"auto"}}>
        ...
        {page==="profile" && (
          <div>
            <PageHeader title="My Profile" subtitle="View your personal and protected information."/>
            <div style={{display:"flex", borderBottom:"1px solid var(--line)", marginBottom:20}}>
              <div className={`df-tab ${profileTab==="basic"?"active":""}`} onClick={()=>setProfileTab("basic")}>Basic Information</div>
              <div className={`df-tab ${profileTab==="private"?"active":""}`} onClick={()=>setProfileTab("private")}>Private Information</div>
            </div>
            {profileTab==="basic" && <BasicInfoGrid emp={me}/>}
            {profileTab==="private" && (
              loadingPrivate ? (
                <EmptyState title="Loading private information..." subtitle="Please wait while we retrieve your protected record."/>
              ) : privateError && !privateInfo ? (
                <EmptyState title="Unable to load private information" subtitle={privateError}/>
              ) : privateInfo ? (
                <PrivateInfoGrid emp={{ ...me, privateInfo }}/>
              ) : (
                <EmptyState title="No private information found" subtitle="No additional records available."/>
              )
            )}
          </div>
        )}
        ...
        {page==="timeoff" && (
          <div>
            ...
            <LeaveHistoryTable rows={myLeave} allocations={allocations}/>
          </div>
        )}
        {page==="payroll" && (
          <div>
...
function RequestTimeOffModal({ allocations, onClose, onSubmit }){
  ...
  function submit(e){
    e.preventDefault();
    if (!form.startDate || !form.endDate || !form.reason){ setErr("Start date, end date, and reason are required."); return; }
    if (form.endDate < form.startDate){ setErr("End date can't be before the start date."); return; }
    onSubmit(form);
  }
  return (
    <Modal onClose={onClose} width={460}>
      ...
      <form onSubmit={submit}>
        <div style={{padding:22, display:"flex", flexDirection:"column", gap:14}}>
          ...
          <div>
            <label className="df-label">Attach document (optional)</label>
        ...
        <div style={{padding:22, display:"flex", gap:10, justifyContent:"flex-end", borderTop:"1px solid var(--line)"}}>
          <button type="button" className="df-btn df-btn-ghost" onClick={onClose}>Cancel</button>
          <button type="submit" className="df-btn df-btn-dawn">Submit request</button>
        </div>
      </form>
    </Modal>
  );
